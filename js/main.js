...
function applyUnifiedWaveShader(material) {
...
function fadeRoot(root, toVisible, duration = 5.0) { // <-- Set 5.0 seconds default
  if (!root) return;
  prepareRootForFade(root);
  
  // Make sure incoming root is visible right away for overlapping dissolve
    ...
    mats.forEach(mat => {
      ...
      if (toVisible) {
        ...
        gsap.to(mat, {
          ...
          duration: duration,
          ease: "sine.inOut", // Smooth cross-fade curve
          onComplete: () => {
        ...
        gsap.to(mat, {
          ...
          ease: "sine.inOut",
          onComplete: () => { 
            c.visible = false; // Only hide after 100% finished
          }
        });
      }
    });
...
function setBlockVariantMode(blockId, mode, { animate = true } = {}) {
  ...
  if (animate) {
    const DISSOLVE_TIME = 5.0; // Set transition time (in seconds)

    // 1. Force both models to be visible simultaneously at frame 0 to prevent a mid-transition gap
    pair.root.visible = true;
    pair.duplicate.visible = true;

    // 2. Cross-dissolve both models simultaneously over 5 seconds
    fadeRoot(pair.root, !showPollution, DISSOLVE_TIME);
    fadeRoot(pair.duplicate, showPollution, DISSOLVE_TIME);
  } else {
...
function handleGLB(url, root) {
  ...
  if      (file.startsWith('ground') || file.startsWith('gnd'))     { baseGround=root; scene.add(root); }
    ...
    root.traverse(c => {
      if (c.isMesh) {
        applyUnifiedWaveShader(c.material);
